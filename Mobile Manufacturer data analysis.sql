...
Create new database
************************************************************************************************/
CREATE DATABASE db_SQLCaseStudies2
...
select IDModel[Cellphone ModelID], IDCustomer, [Date], [State] from FACT_TRANSACTIONS ft join DIM_LOCATION dl on ft.IDLocation = dl.IDLocation
...
select Country,[State],sum(Quantity)[Quantity] from FACT_TRANSACTIONS ft join DIM_LOCATION dl on ft.IDLocation = dl.IDLocation
...
select distinct IDModel, ZipCode,[State],TotalPrice, Quantity from FACT_TRANSACTIONS ft join DIM_LOCATION dl on ft.IDLocation = dl.IDLocation
...
select Model_Name[Cellphone Name], min(Unit_price)[Cellphone Price] from DIM_MODEL
group by Model_Name
order by [Cellphone Price]
...
select distinct top 5 Manufacturer_Name,  round(avg(TotalPrice),2)[Average Price], count(Quantity)[Sales Quantity] from FACT_TRANSACTIONS ft join DIM_MODEL dm on ft.IDModel = dm.IDModel
...
select Customer_Name, avg(TotalPrice)[Average Spending] from DIM_CUSTOMER dc join FACT_TRANSACTIONS ft on dc.IDCustomer = ft.IDCustomer
...
Select top 5 Model_Name, Quantity from FACT_TRANSACTIONS ft join DIM_MODEL dm on ft.IDModel = dm.IDModel
...
Select top 2 Manufacturer_Name, TotalPrice, [Date][2009]  from FACT_TRANSACTIONS ft join DIM_MODEL dm on ft.IDModel = dm.IDModel
								join DIM_MANUFACTURER de on dm.IDManufacturer = de.IDManufacturer
...
select top 100 IDCustomer, avg(TotalPrice)[Average Spend], avg(Quantity)[Average Quantity], year(date)[Year], avg(TotalPrice)*100/(select avg(TotalPrice) from FACT_TRANSACTIONS)[Percentage of Change in Spend]
